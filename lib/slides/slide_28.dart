...
class _Slide28State extends State<Slide28> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ Tu imagen va a la DERECHA (cambia el path cuando quieras)
    const rightImg = "assets/slide28/slide28.png";
                    ...
                    children: [
                      ...
                      Expanded(
                        child: LayoutBuilder(
                          builder: (context, c) {
                            final w = c.maxWidth;
                            final stacked = w < 1100 * s; // responsive -> apila
                            ...
                            final left = SlideTransition(
                              ...
                              child: _GlassCard(
                                ...
                                child: _LeftArchitectureTable(s: s),
                              ),
                            );
...
class _LeftArchitectureTable extends StatelessWidget {
  final double s;
  const _LeftArchitectureTable({required this.s});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // ✅ Antes: Text("Componente", style: title),
        // ✅ Ahora: título con padding + fitted para que no se corte jamás
        Padding(
          padding: EdgeInsets.only(
            left: 13 * s, // 👉 más a la derecha
            top: 9 * s, // 👉 más abajo
            right: 6 * s,
          ),
          child: SizedBox(
            ...
            child: FittedBox(
              ...
              child: Text("Componente", style: title),
            ),
          ),
        ),
                    ...
                    child: DefaultTextStyle(
                      ...
                      child: Column(
                        children: const [
                          _RowItem(
                            comp: "ctrlX CORE",
                            func: "Orquestador de la red y diagnóstico web.",
                          ),
                          _RowItem(
                            comp: "SAFEX C.12 / C.15",
                            func:
                                "El cerebro lógico que escala mediante módulos de expansión.",
                          ),
                          _RowItem(
                            comp: "ctrlX DRIVE",
                            func:
                                "Ejecutor de funciones SafeMotion (STO, SLS, SOS).",
                          ),
                          _RowItem(
                            comp: "EtherCAT / FSoE",
                            func:
                                "El “bus” que permite que la seguridad viaje por toda la planta.",
                          ),
                        ],
                      ),
                    ),
        ...
        SizedBox(height: 10 * s),
        Text(
          "Función en la arquitectura escalable",
          style: title.copyWith(fontSize: 18.0 * s),
        ),
      ],
    );
  }
...
class _TableHeaderRow extends StatelessWidget {
  final double s;
  const _TableHeaderRow({required this.s});
    ...
    return _InnerGlassPanel(
      ...
      child: Row(
        children: [
          SizedBox(
            width: 165 * s,
            child: Text("Componente", style: th),
          ),
...
class _RowItem extends StatelessWidget {
  final String comp;
  final String func;
  const _RowItem({required this.comp, required this.func});

  @override
  Widget build(BuildContext context) {
    final s =
        (context.findAncestorWidgetOfExactType<_LeftArchitectureTable>()?.s) ??
        1.0;
