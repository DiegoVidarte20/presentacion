...
class _Slide30State extends State<Slide30> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    const rightImg = "assets/slide30/slide30.png"; // <-- tu imagen derecha
                          ...
                          builder: (context, c) {
                            ...
                            final left = SlideTransition(
                              ...
                              child: _GlassCard(
                                ...
                                child: _LeftVerticalIntegration(s: s),
                              ),
                            );
...
class _LeftVerticalIntegration extends StatelessWidget {
  final double s;
  const _LeftVerticalIntegration({required this.s});
  ...
  Widget build(BuildContext context) {
    ...
    const paragraph =
        "La integración vertical consiste en lograr que los datos fluyan sin interrupciones "
        "desde el sensor en el piso de planta hasta los sistemas de decisión en la oficina "
        "(o la nube). Se rompen los \"silos\" de información para que el nivel jerárquico "
        "superior sepa exactamente qué pasa en el inferior.";

    const rows = [
      _NVRow(n: "5. ERP / Cloud", f: "Gestión global, IA y finanzas."),
      _NVRow(n: "4. MES", f: "Ejecución de manufactura y gestión de planta."),
      _NVRow(n: "3. SCADA / HMI", f: "Monitoreo y control visual de procesos."),
      _NVRow(n: "2. Control", f: "Lógica de control y tiempo real."),
      _NVRow(n: "1. Campo", f: "Sensores y actuadores físicos."),
    ];

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // ✅ “título/padding” como pediste (para que no se corte nada)
        Padding(
          padding: EdgeInsets.only(left: 8 * s, top: 6 * s, right: 6 * s),
          child: SizedBox(
            width: double.infinity,
            child: FittedBox(
              fit: BoxFit.scaleDown,
              alignment: Alignment.centerLeft,
              child: Text("Vertical integration", style: title),
            ),
          ),
        ),
        ...
        _InnerGlassPanel(
          ...
          child: Text(paragraph, style: base),
        ),
              ...
              children: [
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      children: rows.map((e) => _LevelRow(s: s, level: e.n, func: e.f)).toList(),
                    ),
                  ),
                ),
              ],
...
class _NVRow {
  final String n;
  final String f;
  const _NVRow({required this.n, required this.f});
}

class _LevelHeaderRow extends StatelessWidget {
  final double s;
  const _LevelHeaderRow({required this.s});
  ...
  Widget build(BuildContext context) {
    ...
    return _InnerGlassPanel(
      ...
      child: Row(
        children: [
          SizedBox(width: 180 * s, child: Text("Nivel", style: th)),
          Expanded(child: Text("Función", style: th)),
        ],
      ),
    );
  }
...
class _LevelRow extends StatelessWidget {
  ...
  final String level;
  final String func;
  const _LevelRow({required this.s, required this.level, required this.func});
